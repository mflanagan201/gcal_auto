...
sort_contribution<-sort(data.frame(CONTRIBUTION[72,1:ncol(CONTRIBUTION)]), decreasing = FALSE) %>% data.frame()
...
exclude_top<-colnames((sort(x[200,],decreasing=TRUE))[1:23]) 
exclude_bottom<-colnames((sort(x[200,],decreasing=FALSE))[1:23])
...
for(i in 1:(nrow(x))){
DF[i,]  <-t(data.frame(colnames(sort(data.frame(x[i]),decreasing=TRUE)))) 
}
...
for(i in 1:(nrow(x))){
  DF_decreased[i,]  <-t(data.frame(colnames(sort(data.frame(x[i]),decreasing=FALSE)))) 
}
...
```{r isolate series, include=FALSE}

All_Index_Hicp<-SERVICES_GOODS_XTS_INDEX

names(All_Index_Hicp)<-c(Index_names)

All_Weight_Hicp<-TOTAL_WEIGHT
names(All_Weight_Hicp)<-c(Index_names)

Unchained_Index<-matrix(NA, ncol=ncol(SERVICES_GOODS_XTS_INDEX["2016/"]),nrow=nrow(SERVICES_GOODS_XTS_INDEX["2016/"])) %>% data.frame()
for(i in 1:ncol(SERVICES_GOODS_XTS_INDEX["2016/"])){
Unchained_Index[,i]<-100*(Chainlinking(All_Index_Hicp[,i]))  
}

names(Unchained_Index)<-c(Index_names)


test_isolate_1<-c("Electricity","Actual Rent")

test_isolate_names<-NA
test_exclusion_names<-NA


for (i in 1:length(Index_names)){
if(Index_names[i]!=test_isolate_1){
  test_isolate_names[i]<-Index_names[i]
} else {
  test_exclusion_names[i]<-Index_names[i]
}
}

test_isolate_names<-na.omit(test_isolate_names)

isolated_weight<-data.frame(matrix(NA,ncol=ncol(All_Weight_Hicp) ,nrow=nrow(All_Weight_Hicp)))
for (i in 1:ncol(All_Weight_Hicp)){
  if(names(All_Weight_Hicp[i]) != test_isolate_1){
    isolated_weight[i]<-All_Weight_Hicp[i]
  }
}
 
isolated_weight[is.na(isolated_weight)]<-0
All_Index_Hicp<-data.frame(Unchained_Index)
names(All_Index_Hicp)<-c(Index_names)


Adj_Weight_HICP<-All_Weight_Hicp[test_isolate_names]/rowSums(All_Weight_Hicp[test_isolate_names]) 
Adj_index_HICP<-All_Index_Hicp[test_isolate_names] %>% as.xts(index(SERVICES_GOODS_XTS_INDEX["2016/"]))


test_con<-as.numeric(Adj_index_HICP["2017/"])*as.xts(Adj_Weight_HICP) 
test_con[is.nan(test_con)]<-0
aggregate_isolated_index<-rowSums(na.locf(test_con, na.rm = TRUE)) %>% as.xts(index(Adj_index_HICP["2017/"]))

aggregate_isolated_index_FINAL<-matrix(NA,nrow=nrow(aggregate_isolated_index["2017/"]),ncol=ncol(aggregate_isolated_index["2017/"])) %>% as.xts(order.by=index(OVERALL_HICP_XTS["2017/"])[1:length(aggregate_isolated_index)])
for (i in 1:length(aggregate_isolated_index["2017"])){
aggregate_isolated_index_FINAL["2017"][i]<-OVERALL_HICP_XTS["2016-12-01"]*as.numeric(aggregate_isolated_index["2017"][i])/100
}


aggregate_isolated_index<-aggregate_isolated_index/100
for (i in 1:nrow(aggregate_isolated_index_FINAL["2018/"])){
aggregate_isolated_index_FINAL["2018/"][i,]<-as.numeric((aggregate_isolated_index["2018/"][i,]))*as.numeric(aggregate_isolated_index_FINAL[paste0(year(aggregate_isolated_index_FINAL["2018/"][i])-1, "-12-01")])
}
...
Contribution_highest_name<-sort(last(summary_contribution[1:5]))[(ncol(summary_contribution[1:5]))] %>% names()

Contribution_highest_number<-sort(last(summary_contribution[1:5]))[(ncol(summary_contribution[1:5]))] %>% round(1)
...
sub_compon_max<-sort(last(CONTRIBUTION),decreasing = TRUE)[1] %>% round(1)
...
ORDER_SERVICES_CONTRIBUTION<-last(CONTRIBUTION[1:36]) %>% data.frame() %>% 
...
```
`r embed_data(Inflation_contributions_graph_data,filename="Inflation_contributions_graph_data.csv.csv")` 
<br>
<br>
...
Last_month_decreasing_component<-paste0(lag(GROWTH_COMPONENTS[names(sort(LATEST_Contribution_CHANGE)[1])],1)[nrow(GROWTH_COMPONENTS),],sep="%")
Current_month_decreasing_component<-paste0(GROWTH_COMPONENTS[names(sort(LATEST_Contribution_CHANGE)[1])][nrow(GROWTH_COMPONENTS),],sep="%")


Last_month_increasing_component<-paste0(lag(GROWTH_COMPONENTS[names(sort(LATEST_Contribution_CHANGE,decreasing = TRUE)[1])],1)[nrow(GROWTH_COMPONENTS),],sep="%")
Current_month_increasing_component<-paste0(GROWTH_COMPONENTS[names(sort(LATEST_Contribution_CHANGE,decreasing = TRUE)[1])][nrow(GROWTH_COMPONENTS),],sep="%")
...
text3<-names(sort(LATEST_Contribution_CHANGE)[1])
text4<-names(sort(LATEST_Contribution_CHANGE,decreasing = TRUE)[1])

```
`r embed_data(Waterfall_graph_data, filename="Waterfall_graph_data.csv.csv")` 
...
exclusion_max<-trimming_series[nrow(trimming_series)] %>% data.frame() %>% sort(decreasing = TRUE) %>% round(1) 
...
exclusion_min<-trimming_series[nrow(trimming_series)] %>% data.frame() %>% sort(decreasing = FALSE) %>% round(1) 
```
...
```
`r embed_data(trimmed_graph_data, filename="trimmed_graph_data.csv.csv")` 
...
```
`r embed_data(graph_data_inflation_basket, filename="graph_data_inflation_basket.csv.csv")` 
...
`r embed_data(over_five_12_Months, filename="Above_5_per_cent_12_months.csv.csv")` 
...
DFF<-DF[names(sort(CONTRIBUTION[nrow(CONTRIBUTION),],decreasing = TRUE)[1:20])]
...
GG_heatmap<-ggplot(data=DFFF, aes(y=variable,x=index.SERVICES_GOODS_XTS_INDEX_GROWTH..2021....,fill=value, label=value))+
            ...
            linetype = 0)+geom_text(aes(label = value), color = "black", size = 2) +scale_fill_gradient(low = "green", high = "red")+  theme(legend.position = "none")
...
NAMES_HIGH_HEATMAP<-sort(Graph_Data_Heatmap[nrow(Graph_Data_Heatmap),],decreasing = TRUE)[1] %>% names()
heatmap_date<-last(unique(DFFF$index.SERVICES_GOODS_XTS_INDEX_GROWTH..2021....))

text_heatmap_growth_rate<-sort(Graph_Data_Heatmap[nrow(Graph_Data_Heatmap),],decreasing = TRUE)[1] %>% as.numeric() %>% paste0(sep="%")

NAMES_HIGH_HEATMAP_2<-sort(Graph_Data_Heatmap[nrow(Graph_Data_Heatmap),],decreasing = TRUE)[2] %>% names()
text_heatmap_growth_rate_2<-sort(Graph_Data_Heatmap[nrow(Graph_Data_Heatmap),],decreasing = TRUE)[2] %>% as.numeric() %>% paste0(sep="%")

NAMES_HIGH_HEATMAP_3<-sort(Graph_Data_Heatmap[nrow(Graph_Data_Heatmap),],decreasing = TRUE)[3] %>% names()
text_heatmap_growth_rate_3<-sort(Graph_Data_Heatmap[nrow(Graph_Data_Heatmap),],decreasing = TRUE)[3] %>% as.numeric() %>% paste0(sep="%")

       
```

`r embed_data(Graph_Data_Heatmap, filename="Graph_Data_Heatmap.csv.csv")`
...
IRE_EA_DIFF<-data.frame(index(EA_INFLATION["2020/"]), LINE_DATA$Growth[1:length(EA_INFLATION["2020/"])]-EA_INFLATION["2020/"]) 
...
```
`r embed_data(Graph_Data_EA, filename="Graph_Data_EA.csv.csv")`
...
`r embed_data(expectations_graph_data, filename="expectations_graph_data.csv.csv")`
...
```{r Commodities, warning=FALSE, include=FALSE}
library(tidyquant)

tickers<- c("GC=F","SI=F","PL=F","HG=F","NG=F","ZC=F","ZM=F","HO=F","KE=F","LBS=F","SB=F","PA=F","ALI=F","RB=F","BZ=F","HO=F","NG=F")


Data  <- tq_get(tickers, get = "stock.prices", from = " 2000-01-01")

df<- Data %>% group_by(symbol) %>% mutate(x=ifelse(symbol))

#energy commodities
Natural_GAS_Futures<- subset(df, symbol=="NG=F") %>% select(-x) 
Gasoline_Futures<- subset(df, symbol=="RB=F") %>% select(-x) 
Brent_crude_oil_Futures<- subset(df, symbol=="BZ=F") %>% select(-x) 
heating_oil_Futures<- subset(df, symbol=="HO=F") %>% select(-x) 




Gold_Futures<-subset(df, symbol=="GC=F") %>% select(-x)
Silver_Futures<-subset(df, symbol=="SI=F") %>% select(-x)
Platinum_Futures<- subset(df, symbol=="PL=F") %>% select(-x)
Copper_Futures<- subset(df, symbol=="HG=F") %>% select(-x) 
Corn_Futures<- subset(df, symbol=="ZC=F") %>% select(-x) 
Soybean_Meal_Futures<- subset(df, symbol=="ZM=F") %>% select(-x) 
Heating_Oil_Futures<- subset(df, symbol=="HO=F") %>% select(-x) 
Wheat_Futures<- subset(df, symbol=="KE=F") %>% select(-x) 
Random_Length_Lumber_Futures<- subset(df, symbol=="LBS=F") %>% select(-x)
Sugar_Futures<- subset(df, symbol=="SB=F") %>% select(-x)
Palladium_Futures<- subset(df, symbol=="PA=F") %>% select(-x)
Aluminum_Futures<- subset(df, symbol=="ALI=F") %>% select(-x)


Natural_GAS_Futures_xts<-as.xts(Natural_GAS_Futures$close, order.by =Natural_GAS_Futures$date)
Natural_GAS_Futures_xts<-Natural_GAS_Futures_xts[!duplicated(Natural_GAS_Futures_xts)]
Gasoline_Futures_xts<-as.xts(Gasoline_Futures$close, order.by =Gasoline_Futures$date)
Brent_crude_oil_Futures_xts<-as.xts(Brent_crude_oil_Futures$close, order.by =Brent_crude_oil_Futures$date)
heating_oil_Futures_xts<-as.xts(heating_oil_Futures$close, order.by =heating_oil_Futures$date) 
heating_oil_Futures_xts<-heating_oil_Futures_xts[!duplicated(heating_oil_Futures_xts)]

CHANGE_NAT_GAS_WAR<-100*(as.numeric(last(Natural_GAS_Futures_xts))- Natural_GAS_Futures_xts["2022-02-01"])/Natural_GAS_Futures_xts["2022-02-01"] %>% round(1)

CHANGE_NAT_GAS_BUDGET<-100*(as.numeric(last(Natural_GAS_Futures_xts))- Natural_GAS_Futures_xts["2023-04-18"])/Natural_GAS_Futures_xts["2023-04-18"] %>% round(1)

CHANGE_NAT_GAS_BUDGET<-round(CHANGE_NAT_GAS_BUDGET,1)

CHANGE_Gasoline_WAR<-100*(as.numeric(last(Gasoline_Futures_xts))- Gasoline_Futures_xts["2022-02-01"])/Gasoline_Futures_xts["2022-02-01"] %>% round(1)


CHANGE_Gasoline_BUDGET<-100*(as.numeric(last(Gasoline_Futures_xts))- Gasoline_Futures_xts["2023-04-18"])/Gasoline_Futures_xts["2023-04-18"] %>% round(1)


CHANGE_BRENT_WAR<-100*(as.numeric(last(Brent_crude_oil_Futures_xts))- Brent_crude_oil_Futures_xts["2022-02-01"])/Brent_crude_oil_Futures_xts["2022-02-01"] %>% round(1)


CHANGE_BRENT_BUDGET<-100*(as.numeric(last(Brent_crude_oil_Futures_xts))- Brent_crude_oil_Futures_xts["2023-04-18"])/Brent_crude_oil_Futures_xts["2023-04-18"] %>% round(1)


CHANGE_HEATING_OIL_WAR<-100*(as.numeric(last(heating_oil_Futures_xts))- heating_oil_Futures_xts["2022-02-01"])/heating_oil_Futures_xts["2022-02-01"] %>% round(1)

CHANGE_HEATING_OIL_BUDGET<-100*(as.numeric(last(heating_oil_Futures_xts))- heating_oil_Futures_xts["2023-04-18"])/heating_oil_Futures_xts["2023-04-18"] %>% round(1)


ENERGY_BUDGET_CHANGE<-merge(CHANGE_NAT_GAS_BUDGET,CHANGE_Gasoline_BUDGET,CHANGE_BRENT_BUDGET,CHANGE_HEATING_OIL_BUDGET) %>% data.frame()

colnames(ENERGY_BUDGET_CHANGE)<-c("Natural Gas", "Gasoline","Brent Crude Oil","Heating Oil")

ENERGY_BUDGET_CHANGE<-ENERGY_BUDGET_CHANGE %>% melt() 

```

```{r Energy Graph, echo=FALSE, warning=FALSE}

Gold_Futures_xts<-as.xts(Gold_Futures$close, order.by =Gold_Futures$date)
Silver_Futures_xts<-as.xts(Silver_Futures$close, order.by =Silver_Futures$date)
Platinum_Futures_xts<-as.xts(Platinum_Futures$close, order.by =Platinum_Futures$date)
Copper_Futures_xts<-as.xts(Copper_Futures$close, order.by =Copper_Futures$date)
Natural_GAS_Futures_xts<-as.xts(Natural_GAS_Futures$close, order.by =Natural_GAS_Futures$date)
Corn_Futures_xts<-as.xts(Corn_Futures$close, order.by =Corn_Futures$date)
Soybean_Meal_Futures_xts<-as.xts(Soybean_Meal_Futures$close, order.by =Soybean_Meal_Futures$date)
Heating_Oil_Futures_xts<-as.xts(Heating_Oil_Futures$close, order.by =Heating_Oil_Futures$date)
Wheat_Futures_xts<-as.xts(Wheat_Futures$close, order.by =Wheat_Futures$date)
Random_Length_Lumber_Futures_xts<-as.xts(Random_Length_Lumber_Futures$close, order.by =Random_Length_Lumber_Futures$date)
Sugar_Futures_xts<-as.xts(Sugar_Futures$close, order.by =Sugar_Futures$date)
Palladium_Futures_xts<-as.xts(Palladium_Futures$close, order.by =Palladium_Futures$date)
Aluminum_Futures_xts<-as.xts(Aluminum_Futures$close, order.by =Aluminum_Futures$date)


xts<-cbind(Brent_crude_oil_Futures_xts,Natural_GAS_Futures_xts,Gold_Futures_xts,Silver_Futures_xts,Platinum_Futures_xts,Copper_Futures_xts,Corn_Futures_xts,Soybean_Meal_Futures_xts,heating_oil_Futures_xts, Random_Length_Lumber_Futures_xts,Sugar_Futures_xts,Palladium_Futures_xts,Aluminum_Futures_xts)  %>% na.omit()

COMMODITY_CHANGE<-data.frame(matrix(NA, nrow=1,ncol=ncol(xts)))

for(i in 1:ncol(xts)){
  
  
COMMODITY_CHANGE[,i]<-(last(na.omit(xts[,i]))/(as.numeric(xts["2023-04-18",i]))-1)*100    
}



days<-(Sys.Date()-as.Date("2023-04-18")) %>% as.numeric()


```


```{r Graph text, warning=FALSE, include=FALSE}
CHANGE_BRENT_BUDGET_TEXT<-conditional(CHANGE_BRENT_BUDGET)
CHANGE_NAT_GAS_BUDGET_TEXT<-conditional(CHANGE_NAT_GAS_BUDGET)
