...
Deflated_VAT_MONTHLY_GROWTH<-as.xts(Deflated_VAT_MONTHLY_GROWTH, order.by = index(VAT_YTD_GROWTH))
...
Deflated_VAT_MONTHLY_GROWTH$VAT
summary_consumer<-merge(retail_Annual$`Retail Sales (y-o-y`,retail_Annual$`Core Retail Sales (y-o-y`,Deflated_VAT_MONTHLY_GROWTH,Deflated_ALL_CB_PAYMENTS_GROWTH,Services_Vol_M_Growth,HH_DEPOSITS_annual_growth,PMOD_XTS_Growth,Unemployment_Rate_XTS,HEADLINE_HICP_GROWTH,CORE_HICP_GROWTH,EXTERNAL_TRADE_VOL_XTS_Growth,EXTERNAL_TRADE_VAL_XTS_Growth) %>% .["2024-01-01/"]  %>% data.frame() %>% round(1) %>% t() 
