...
YTD <- function(x) {
  ...
  if (length(CURRENTYEAR_YTD) == 0 || length(LASTYEAR) == 0) {
    return(NA)  # Handle cases where no data exists for the current or previous year
  }

  YTD_CURRENTYEAR <- CURRENTYEAR_YTD
  for (i in 1:length(CURRENTYEAR_YTD)) {
    YTD_CURRENTYEAR[i] <- 
      ((as.numeric(sum(as.numeric(CURRENTYEAR_YTD[1:i]))) / 
        as.numeric(sum(as.numeric(LASTYEAR[1:i])))) - 1) * 100
  }
  return(YTD_CURRENTYEAR)
}
...
HICP_QUARTERLY_GROWTH<-growth(HICP_QUARTERLY,4)
 ...
 YTD <- function(x){
   ...
   for(i in 1:length(CURRENTYEAR_YTD)){     YTD_CURRENTYEAR[i]<-((as.numeric(sum(as.numeric(CURRENTYEAR_YTD[1:i])))/as.numeric(sum(as.numeric(LASTYEAR[1:i]))))-1)*100  
   }  
   return(YTD_CURRENTYEAR)
   }
...
VAT_YTD_GROWTH<-rbind(YTD(VAT_XTS["/2023"]),YTD(VAT_XTS["2024/"]))
