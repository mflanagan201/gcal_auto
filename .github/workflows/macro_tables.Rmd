...
TAX_URL<-c("https://databank.finance.gov.ie/FinDataBank.aspx?rep=TaxMthTrend")
TAX<-read_table(TAX_URL) 
...
TABLEEXTRACTION <- function(tablename, tablerows, DATES_DOF_EXCHE, url_formats,indicator) {
  ...
  for (date in DATES_DOF_EXCHE) {
    ...
    for (url_format in url_formats) {
      ...
      response_page <- GET(publication_page_url, handle = session_handle, base_headers, timeout(60))
      ...
      response_pdf <- GET(pdf_url_full, handle = session_handle,
                          ...
                          timeout(120)) # Increase timeout for large PDF downloads
...
#      table1<- table1[, rev(seq_len(ncol(table1))), drop = FALSE]
      all_extracted_tables[[as.character(date)]] <- table1
 ...
 #     if (any(table1[[1]] %like any% c(paste(indicator)), na.rm = TRUE)) {
  ...
  return(list(extracted_tables = all_extracted_tables,indicator_row=indicator_table))
  
}

table2<-TABLEEXTRACTION("Analysis of Net Voted Expenditure",100,DATES_DOF_EXCHE,url_formats,c("%Cumulative Voted Spending%","%Total Net cumulative Voted Spending%","%Total Net Cumulative Voted Spending%"))


annual_growth_val<-matrix(NA, nrow=length(table2$extracted_tables),ncol=2)

for(i in 1:length(table2$extracted_tables)){ 
          if(format(as.Date(paste0("01-", DATES_DOF_EXCHE[i]), format = "%d-%B-%Y"),"%B") == c("January")){
            votedspending_df<-(str_split_fixed(str_squish(stri_reverse(table2$indicator_row[i])), " ", n = 5))%>% stri_reverse()  %>% data.frame() %>% t() %>% .[ncol(.):1] %>% matrix(.,ncol = 5) %>% data.frame()
          names(votedspending_df)<-c("name","currentFY","lastFY","yoyFY","yoygrowthFY")
          annual_growth_val[i,1] <- votedspending_df$yoygrowthFY 
          
          } else {
            votedspending_df<-(str_split_fixed(str_squish(stri_reverse(table2$indicator_row[i])), " ", n = 11))%>% stri_reverse() %>% data.frame() %>% t() %>% .[ncol(.):1] %>% matrix(.,ncol = 11) %>% data.frame()
...
CT_TABLE<-TABLEEXTRACTION("Analysis of Taxation Receipts",25,DATES_DOF_EXCHE,url_formats,c("%Corporation Tax%"))
            
CT_annual_growth<-matrix(NA, nrow=length(CT_TABLE$indicator_row),ncol=1)

for(i in 1:length(CT_TABLE$extracted_tables)){ 
    votedspending_df<-(str_split_fixed(str_squish(stri_reverse(CT_TABLE$indicator_row[i])), " ", n = 5)) %>% stri_reverse()  %>% data.frame() %>% t() %>% .[ncol(.):1] %>% matrix(.,ncol = 5) %>% data.frame()
    names(votedspending_df)<-c("name","currentFY","lastFY","yoyFY","yoygrowthFY")
    CT_annual_growth[i,1] <- votedspending_df$yoygrowthFY 
}

Fiscal_indicators_xts<-data.frame(annual_growth_val,CT_annual_growth) %>% as.xts(order.by=as.Date(paste0("01-", DATES_DOF_EXCHE), format = "%d-%B-%Y"))
colnames(Fiscal_indicators_xts)<-c("gross expenditure","expenditure overruns/underruns", "corporation tax")
...
Exchequer_Statement<-TABLEEXTRACTION("Analytical Exchequer Statement",40,DATES_DOF_EXCHE,url_formats,c("%: tax revenue%"))


tax_revenue_annual_growth<-matrix(NA, nrow=length(Exchequer_Statement$extracted_tables),ncol=1)
for(i in 1:length(Exchequer_Statement$extracted_tables)){ 
  votedspending_df<-(str_split_fixed(str_squish(stri_reverse(Exchequer_Statement$indicator_row[i])), " ", n = 5)) %>% stri_reverse()  %>% data.frame() %>% t() %>% .[ncol(.):1] %>% matrix(.,ncol = 5) %>% data.frame()
  names(votedspending_df)<-c("name","currentFY","lastFY","yoyFY","yoygrowthFY")
  tax_revenue_annual_growth[i,1] <- votedspending_df$yoygrowthFY 
}


Exchequer_Statement_Expenditure<-TABLEEXTRACTION("Analytical Exchequer Statement",40,DATES_DOF_EXCHE,url_formats,c("%: tax revenue%"))
...
Fiscal_indicators_xts<-data.frame(str_replace(annual_growth_val[,1],"%",""),(annual_growth_val[,2]),str_replace(tax_revenue_annual_growth,"%",""),str_replace(CT_annual_growth,"%","")) %>% as.xts(order.by=as.Date(paste0("01-", DATES_DOF_EXCHE), format = "%d-%B-%Y"))
colnames(Fiscal_indicators_xts)<-c("gross expenditure","expenditure overruns/underruns", "Tax Revenue","corporation tax")




summary_consumer<-merge(retail_Annual$`Retail Sales (y-o-y`,retail_Annual$`Core Retail Sales (y-o-y`,Deflated_VAT_MONTHLY_GROWTH$VAT,Deflated_ALL_CB_PAYMENTS_GROWTH,Services_Vol_M_Growth,HH_DEPOSITS_annual_growth,PMOD_XTS_Growth,Unemployment_Rate_XTS,HEADLINE_HICP_GROWTH,CORE_HICP_GROWTH,EXTERNAL_TRADE_VOL_XTS_Growth,EXTERNAL_TRADE_VAL_XTS_Growth,Fiscal_indicators_xts$`gross expenditure`,Fiscal_indicators_xts$`expenditure overruns/underruns`,Fiscal_indicators_xts$`Tax Revenue`,Fiscal_indicators_xts$`corporation tax`)





) %>% .["2024-10-01/"]  %>% data.frame() %>% round(1) %>% t() 
...
colnames(DOWNLOAD_MONTHLY_DF)<-c("Date","Retal Sales Index","Core Retal Sales Index","VAT","Central Bank Card Payment","Modified Service Index","Household Deposits","Payroll Employment","Unemployment Rate","Headline Inflation","Core Inflation","External Trade VOlume Index", "External Trade Value")
