...
DATES_DOF_EXCHE_CHECK<-data.frame(matrix(NA,nrow=length(DATES_DOF_EXCHE),ncol=1))
...
for (date in DATES_DOF_EXCHE) {
 for (url_format in url_formats) {
   ...
   Sys.sleep(runif(1, min = 3, max = 10))
   if(url_exists(url)){
   DATES_DOF_EXCHE_CHECK[date,]<-paste(DATES_DOF_EXCHE[date,])
   }
 }
}
...
Fiscal_indicators_xts<-data.frame(annual_growth_val,CT_annual_growth) %>% as.xts(order.by=as.Date(paste0("01-", DATES_DOF_EXCHE_CHECK), format = "%d-%B-%Y"))
