...
VAT_MONTHLY <- read_excel("VAT_MONTHLY.xlsx")
VAT_DATE<-as.Date(VAT_MONTHLY$DATE)
VAT_XTS<-as.xts(VAT_MONTHLY$VAT, order.by =VAT_DATE )
...
Deflated_VAT_MONTHLY_GROWTH<-as.xts(Deflated_VAT_MONTHLY_GROWTH,order.by=as.Date(seq(from=as.Date("01/01/2023"), length.out=length(Deflated_VAT_MONTHLY_GROWTH), by="month")))
...
Quarterly_TABLE2<-kbl(Quarterly_consumer_Table[,1:ncol(Quarterly_consumer_Table)], caption = "", booktabs = TRUE, bold=TRUE, colour=DOF_colours[1],align = "c",format = "html") %>%
  ...
  add_footnote( c("all growth rates are in year on year terms.", "VAT is expressed in YTD terms given the seasonality in the data. VAT is deflated by YTD HICP.","Real Disposable income deflated HICP."))    %>% as_image()  
Quarterly_TABLE2
...
Monthly_TABLE2<-kbl(summary_consumer,caption = "", booktabs = TRUE, bold=TRUE, colour=DOF_colours[1],align = "c",format = "html") %>% 
  ...
  add_footnote( c("all growth rates are in year on year terms.", "VAT is expressed in YTD terms given the seasonality in the data. VAT is deflated by YTD HICP."))      %>% as_image()
