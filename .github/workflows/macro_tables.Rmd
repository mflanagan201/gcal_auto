...
CB_URL_Monthly_download_text1<-(CB_URL_Monthly_download_text[[1]])   %>% html_attrs() %>% data.frame() %>% .$. %>% .[2]
...
URL_CENTRAL_BANK_DATA_DOWNLOAD_CARDS<-paste0("https://opendata.centralbank.ie/dataset/19af4a3e-ed9f-41ee-ad9b-52dc72354a15/resource/",CB_URL_Monthly_download_text1,"/download/mpci_cardissuance.csv")
CB_CARD_NUM_DATA<-read_csv(URL_CENTRAL_BANK_DATA_DOWNLOAD_CARDS) %>% data.frame()
CB_DATA_CARD_CC_NUM<-subset(CB_CARD_NUM_DATA,CRD_FNCTN_DESC =="Credit card")
CB_DATA_CARD_DC_NUM<-subset(CB_CARD_NUM_DATA,CRD_FNCTN_DESC =="Debit card")

DC_VOLUME<-as.numeric(CB_DATA_CARD_DC_NUM$OBSERVATION_FREE) %>% na.omit() %>% as.xts(order.by=as.Date(format(CB_DATE, "%Y-%m-01")))
CC_VOLUME<-as.numeric(CB_DATA_CARD_CC_NUM$OBSERVATION_FREE) %>% na.omit() %>% as.xts(order.by=as.Date(format(CB_DATE, "%Y-%m-01")))

CC_DD_VOLUME<-DC_VOLUME+DC_VOLUME
CARD_PAYMENT_PER_CARD<-(DOMESTIC_PAYMENT_ALL["2022-10-01/"]/CC_DD_VOLUME["2022-10-01/"])

CARD_PAYMENT_PER_CARD_GROWTH<-CARD_PAYMENT_PER_CARD %>% growth(freq=12)

ALL_CB_PAYMENTS_QUARTERLY<-CARD_PAYMENT_PER_CARD %>% ts(start=c(2022,10), frequency=12) %>% as.quarterly(FUN = sum) %>% as.xts()
...
Deflated_ALL_CB_PAYMENTS_GROWTH<-((1+(CARD_PAYMENT_PER_CARD_GROWTH/100))/(1+(HICP_SHORT_GROWTH/100))-1)*100
...
419228,4051134,507747,3074026,266528,3522000,168000,3260000,381000) 
...
GGR<- t(data.frame(subset(subset(cso_get_data("GFQ01"),Statistic=="General Government Transactions ESA 2010: Revenue, Expenditure, Financing and Deficit"),Item =="General Government transactions - Revenue - ESA2010 Code (TR)")))%>%  as.numeric() %>% na.omit()
Date_GGR<-seq(as.Date("2000-01-01"), length.out=(length(na.omit(GGR))), by="quarter")
GGR_XTS<-na.omit(GGR) %>% na.omit() %>% as.xts(order.by=Date_GGR)
GGR_XTS_Growth<-growth(GGR_XTS, freq=4) %>% round(1)
colnames(GGR_XTS_Growth)<-c("GGR")


GGE<- t(data.frame(subset(subset(cso_get_data("GFQ01"),Statistic=="General Government Transactions ESA 2010: Revenue, Expenditure, Financing and Deficit"),Item =="General Government transactions - Expenditure - ESA2010 Code (TE)")))%>%  as.numeric() %>% na.omit()
Date_GGE<-seq(as.Date("2000-01-01"), length.out=(length(na.omit(GGE))), by="quarter")
GGE_XTS<-na.omit(GGE) %>% na.omit() %>% as.xts(order.by=Date_GGE)
GGE_XTS_Growth<-growth(GGE_XTS, freq=4) %>% round(1)
colnames(GGE_XTS_Growth)<-c("GGE")


GGB<- t(data.frame(subset(subset(cso_get_data("GFQ01"),Statistic=="General Government Transactions ESA 2010: Revenue, Expenditure, Financing and Deficit"),Item =="General Government Surplus/Deficit - ESA2010 Code (B9)")))%>%  as.numeric() %>% na.omit()
Date_GGB<-seq(as.Date("2000-01-01"), length.out=(length(na.omit(GGB))), by="quarter")
GGB_XTS<-na.omit(GGB) %>% na.omit() %>% as.xts(order.by=Date_GGB)
colnames(GGB_XTS)<-c("GGB")
#here
QNA_CURRENT_PRICE<- data.frame(subset(cso_get_data("NAQ04"),Statistic=="Expenditure on GNP at Current Market Prices"))%>% select(-Statistic)
Date_95_QNA_CURRENT<-seq(as.Date("1995/01/01"), length.out=(length(QNA_CURRENT_PRICE)-1), by="quarters")
GDP_CURRENT_PRICE<-(data.frame(subset(QNA_CURRENT_PRICE, Sector=="Gross Domestic Product at Market Prices"))) %>%select(-Sector) %>% t() %>%  as.xts(order.by = Date_95_QNA_CURRENT) 
colnames(GDP_CURRENT_PRICE)<-c("GDP")

GDP_CURRENT_PRICE_4Q<-rollsum(GDP_CURRENT_PRICE$GDP,4) %>% as.numeric() %>%  as.xts(.,order.by=as.Date(Date_95_QNA_CURRENT[4:length(Date_95_QNA_CURRENT)]))
colnames(GDP_CURRENT_PRICE_4Q)<-c("GDP")

GGB_GDP_RATIO<-(GGB_XTS$GGB/GDP_CURRENT_PRICE$GDP)*100
...
table2<-TABLEEXTRACTION("Analysis of Gross Voted Expenditure",100,DATES_DOF_EXCHE,url_formats,c("%Total Gross Cumulative Voted Spending%"),"VOTEDSPENDING.XLSX")

write.xlsx(c(table2[[1]][1:length(table2[[1]])],table2$extracted_tables), file="VOTEDSPENDING.xlsx")

table2<-excel_sheet_reader(paste("VOTEDSPENDING.xlsx"))

annual_growth_val<-matrix(NA, nrow=length(table2),ncol=2)

for(i in 1:length(table2)){ 
          if(format(as.Date(paste0("01-", DATES_DOF_EXCHE[i]), format = "%d-%B-%Y"),"%B") == c("January")){
  votedspending_df<-stringr::str_split_fixed((str_squish(stri_reverse(unlist(names(c(table2[[i]]))))))," ",n=5) %>% stri_reverse()  %>% data.frame() %>% t() %>% .[ncol(.):1] %>% matrix(.,ncol = 5) %>% data.frame()
          names(votedspending_df)<-c("name","currentFY","lastFY","yoyFY","yoygrowthFY")
          annual_growth_val[i,1] <- votedspending_df$yoygrowthFY 
          
          } else {
            
  votedspending_df<-stringr::str_split_fixed((str_squish(stri_reverse(unlist(names(c(table2[[i]]))))))," ",n=11) %>% stri_reverse()  %>% data.frame() %>% t() %>% .[ncol(.):1] %>% matrix(.,ncol = 11) %>% data.frame()
...
write.xlsx(c(CT_TABLE[[1]][1:length(CT_TABLE[[1]])],CT_TABLE$extracted_tables), file="ct.xlsx")

CT_TABLE<-excel_sheet_reader(paste("ct.xlsx"))
            
CT_annual_growth<-matrix(NA, nrow=length(CT_TABLE),ncol=1)

for(i in 1:length(CT_TABLE)){ 
votedspending_df<-stringr::str_split_fixed((str_squish(stri_reverse(unlist(names(c(CT_TABLE[[i]]))))))," ",n=5) %>% stri_reverse()  %>% data.frame() %>% t() %>% .[ncol(.):1] %>% matrix(.,ncol = 5) %>% data.frame()
    names(votedspending_df)<-c("name","currentFY","lastFY","yoyFY","yoygrowthFY")
...
write.xlsx(c(Exchequer_Statement[[1]][1:length(Exchequer_Statement[[1]])],Exchequer_Statement$extracted_tables), file="tax_revenue.xlsx")

Exchequer_Statement<-excel_sheet_reader(paste("tax_revenue.xlsx"))

tax_revenue_annual_growth<-matrix(NA, nrow=length(DATES_DOF_EXCHE),ncol=1)

for(i in 1:length(Exchequer_Statement)){ 
  votedspending_df<-stringr::str_split_fixed((str_squish(stri_reverse(unlist(names(c(Exchequer_Statement[[i]]))))))," ",n=4) %>% stri_reverse()  %>% data.frame() %>% t() %>% .[ncol(.):1] %>% matrix(.,ncol = 4) %>% data.frame()
...
Fiscal_indicators_xts<-data.frame(str_replace(annual_growth_val[,1],"%",""),str_replace(annual_growth_val[,2],",",""),str_replace(tax_revenue_annual_growth,"%",""),str_replace(CT_annual_growth,"%","")) %>% as.xts(order.by=as.Date(paste0("01-", DATES_DOF_EXCHE), format = "%d-%B-%Y")) %>% data.frame(index(.),.) %>% as.xts(dateFormat = "Date")
...
rownames(summary_consumer)<-c("Retail Sales","Core Retail Sales","Real VAT","Real Card Payment Per Card","Modified Services","Deposits","Payroll Employment" ,"Unemployment Rate","Inflation","Core Inflation","External Trade Volume","External Trade Value","Gross expenditure","Expend. overrun (€m)", "Tax revenue","Corporation tax")
...
Quarterly_consumer_Table<-merge(Consumption_Q_Annual_growth$Consumption,Deflated_VAT_Quarterly_GROWTH, Retail_Quarterly_yoy_growth,Deflated_ALL_CB_PAYMENTS_GROWTH_QUARTERLY
                                ,Services_Vol_Q_Growth,COE_PER_CAPITA_XTS_Growth,Deflated_GDI_PER_CAPITA_XTS_Growth,Average_Hourly_Earnings_XTS_GROWTH,HH_Deposits_ANNUAL_growth,HICP_QUARTERLY_GROWTH,sr,Employment_XTS_Growth,PMOD_XTS_QUARTERLY_GROWTH,Unemployment_Rate_QUARTERLY,GDP_Q_Annual_growth,EXPORTS_Q_ANNUAL_GROWTH,EXTERNAL_TRADE_VOL_XTS_QUARTERLY_GROWTH,EXTERNAL_TRADE_VAL_XTS_QUARTERLY_GROWTH,GGE_XTS_Growth,GGR_XTS_Growth,GGB_GDP_RATIO) %>% .["2023-10-01/"]   %>% round(1) %>% t()
rownames(Quarterly_consumer_Table)<-c("Consumption","Real VAT (YTD)","Retail Sales","Real Card Payment Per Card","Modified Services","Comp of Employees per person","Real Dispos Income per person","Average Hourly Earnings","Household Deposits", "Inflation Rate", "Savings Rate","Employment Growth", "Payroll Employment", "Unemployment Rate","GDP","Total Exports","External Trade Volume","External Trade Value","GG Expenditure Growth","GG Revenue Growth","GGB Ratio")
...
Quarterly_TABLE2<-kbl(Quarterly_consumer_Table[,1:ncol(Quarterly_consumer_Table)-1], caption = "", booktabs = TRUE, bold=TRUE, colour=DOF_colours[1],align = "c",format = "html") %>%
  ...
  pack_rows("Fiscal Indicators", 19, 21,label_row_css = "background-color: #34495e; color: #fff;") %>% 
  column_spec(1,include_thead = TRUE) %>%
  row_spec(0,bold=TRUE, angle=0, align = "c") %>% column_spec(ncol(Quarterly_consumer_Table)-1,include_thead = FALSE) %>%    
...
webshot::webshot(temp_file2, file = "table_image2.png", cliprect = "viewport", vwidth = 900,vheight = 1100, zoom = 1.2)
...
colnames(DOWNLOAD_QUARTERLY_DF)<-c("Date","consumption","VAT","Retail Sales","Central Bank Card Payment","Modified Services","Compensation of Employees","Population","Gross Disposable Income", "Average Hourly Earnings","Household Deposits","HICP Index","Savings Rate","Employment level","Payroll employment","Unemployment Rate","GDP","Exports","External Trade Vol Index", "External Trade Value","GG Expenditure","GG Revenue","GG Balance")
...
webshot::webshot(temp_file, file = "table_image.png", cliprect = "viewport", vwidth = 900,vheight = 860, zoom = 1.2)
...
Fiscal_indicators_xts$`corporation tax`) %>% .[301:nrow(.),] %>% data.frame() %>%  cbind(as.Date(seq(from=as.Date("1995-01-01"), to=last(index(HEADLINE_HICP_XTS)),by='month')),.) %>% as.xts()

colnames(DOWNLOAD_MONTHLY_DF)<-c("Date","Retal Sales Index","Core Retal Sales Index","VAT","Central Bank Card Payment","Modified Service Index","Household Deposits","Payroll Employment","Unemployment Rate","Headline Inflation","Core Inflation","External Trade VOlume Index", "External Trade Value","Gross expenditure","Expend overrun (€m)", "Tax revenue","Corporation tax")
