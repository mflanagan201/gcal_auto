...
Overall_energy_contribution_line<- data.frame(index(ENERGY["2019/"]), ENERGY["2019/"])
...
Energy_inflation <- as.data.frame(ENERGY_OVERALL_INDEX_XTS) %>% as.xts()

for (i in 1:ncol(ENERGY_OVERALL_INDEX_XTS)){
  Energy_inflation[,]<-growth(as.numeric(ENERGY_OVERALL_INDEX_XTS[,]),12)
}

Energy_inflation_text<-conditional(Energy_inflation[length(Energy_inflation)])
...
SERVICES_MAIN_COMPONENTS<-last(data.frame(SERVICES_CONTRIBUTION))[,order(as.numeric(as.character(last(data.frame(SERVICES_CONTRIBUTION)))),decreasing = TRUE)][,1:4]
